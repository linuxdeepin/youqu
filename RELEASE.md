...
这个版本过程中，我们成功在欧拉社区开源 **[src-openeuler](https://gitee.com/src-openeuler/youqu)** ，后续将以 `patch` 的方式持续在欧拉社区进行代码更新，希望欧拉系统上使用到 YouQu 的同学们能给我们反馈使用情况。

此外，重点对统计 `commit` 功能进行了优化和 `Bug` 修复，此功能目前还处于内部灰度阶段，未来有希望成为 YouQu 的另一个亮点，能自动分析统计每次提交中包含的用例和方法的增删改的数据。
...
- YouQu插件image-center更新：支持Windows和MacOS，图片入参支持多种图片格式，并兼容不带文件后缀名入参方式。[@CCrazyPeter](https://github.com/CCrazyPeter)
